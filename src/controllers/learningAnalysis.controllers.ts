...
export const createAction = async (req: Request, res: Response) => {
    try {
        ...
        if (!req.body.timestamp || !req.body.userId || !req.body.zoneId) { // Valida che campi siano presenti
            return res.status(400).json({ error: "Missing required fields: timestamp, userId, or zoneId." });
        }

        let action: any;

        // Specifiche per i tipi di azioni (puoi espandere con più validazioni per ogni tipo)
        switch (type) { //case per ogni tipo di azione? Non c'è modo migliore??
            case "open_learning_page":
                ...
                if (!openAction.action.lpId || !openAction.action.pageId) {
                    return res.status(400).json({ error: "Missing fields for open_learning_page: lpId or pageId." });
                }
                action = await  Models.OpenPageActionModel.create(openAction);
                res.status(200).json({ message: "Action created successfully", data: action });
            break;
            ...
            case "close_learning_page":
                ...
                if (!closeAction.action.lpId || !closeAction.action.pageId) {
                    return res.status(400).json({ error: "Missing fields for close_learning_page: lpId or pageId." });
                }
            break;
            ...
            case "changed_tab":
                ...
                if (!tabAction.action.lpId || !tabAction.action.pageId) {
                    return res.status(400).json({ error: "Missing fields for changed_tab: lpId or pageId." });
                }
            break;
            ...
            case "changed_page":
                ...
                if (!pageAction.action.oldPageId || !pageAction.action.newPageId) {
                    return res.status(400).json({ error: "Missing fields for changed_page: oldPageId or newPageId." });
                }
            break;
...
export const getAllActions = async (req: Request, res: Response) => {
    try {
        const { userId, zoneId, type, lpId, exerciseType, exerciseId, platform, pageId} = req.query; //estrae valori di userId e zoneId dalla query string dell'URL
        const filter: any = {};
        ...
        if(error instanceof Error){
            res.status(500).json({ error: "Error in returning actions (getAllActions)." });
        }
    }
};
