...
export class ManualAA extends AbstractAlgorithm {
  public async getNextExercise(
    ...
    if (!execNodeInfo.concepts) {
      const conceptmap: PolyglotConceptMap | undefined =
        currentNode.data.conceptmap;

      // TODO: create custom Error
      if (!conceptmap) throw Error("Concept map not defined!");

      const concepts: PolyglotConceptNode[] =
        currentNode.data.execution?.concepts ?? conceptmap.nodes;
    ...
    const currentConcept: PolyglotConceptNode =
      execNodeInfo.concepts[execNodeInfo.conceptIndex];
