...
export const getAbstractAlgorithm = (algo: string, ctx: ExecCtx) => {
  if (!algoMap[algo] || typeof algoMap[algo] !== typeof AbstractAlgorithm)
    throw Error("Abstract Algorithm not set");
...
export const getPathSelectorAlgorithm = (algo: string, ctx: ExecCtx) => {
  if (
    !pathSelectorMap[algo] ||
    typeof pathSelectorMap[algo] !== typeof DistrubutionAlgorithm
  )
