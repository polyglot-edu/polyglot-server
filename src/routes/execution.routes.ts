...
router.post("/alexa/next", async (req, res) => {
  ...
  const satisfiedEdges = currentFlow.edges.filter((edge) =>
    satisfiedConditions.includes(edge.reactFlow.id),
  );
  const possibleNextNodes = satisfiedEdges
    .map((edge) =>
    ...
    .filter(
      (n) =>
        n?.type === "multipleChoiceQuestionNode" || n?.type === "lessonNode",
    );
  const nextNode =
...
router.post(
  "/alexa/submit",
  async (req: Request<{}, any, AlexaSubmitRequestBody>, res) => {
    if (lastExercise?.type === "multipleChoiceQuestionNode") {
      let hasAnswer = req.body.answer !== undefined ? 200 : 400;
      if (hasAnswer === 200) {
        res
          .status(hasAnswer)
          .json({ result: Math.random() > 0.5 ? "Not correct" : "Great job!" });
      } else {
